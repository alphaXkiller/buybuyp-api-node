...
const parseParams = (map) => R.compose(
  R.converge(
    R.merge,
    [
...
const _sanitizeParams = R.pickBy(R.compose(R.not,R.isNil))

// normalizeParams:: Object -> Object - Object
const normalizeParams = (map) => R.mapObjIndexed(
  (val, key) => {
...
const _parseSearchParams = (map) => R.compose(
  R.pick(R.keys(map)),
...
const _parsePaginationParams = R.compose(
  (params) => ({
...
const addPagination = R.curry((db, sql, params) => {
  return R.compose(
    R.concat(sql),
